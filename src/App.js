...
// ─── PERMISSÕES ───────────────────────────────────────────────────────────────
const ADMINS = ["eduvgianni@gmail.com", "isaslvador@gmail.com"];
const canEditUser = (email) => ADMINS.includes((email || "").toLowerCase().trim());
...
function NoPermTooltip({children}){
  const [show,setShow]=useState(false);
  return (
    <div style={{position:"relative",display:"inline-flex"}}
      onMouseEnter={()=>setShow(true)} onMouseLeave={()=>setShow(false)}>
      {children}
      {show&&<div style={{position:"absolute",bottom:"calc(100% + 6px)",left:"50%",transform:"translateX(-50%)",background:"#1a1a2e",border:`1px solid ${T.border}`,borderRadius:8,padding:"6px 10px",fontSize:11,color:T.sub,whiteSpace:"nowrap",zIndex:200,pointerEvents:"none",boxShadow:"0 8px 24px #00000080"}}>
...
// ─── BOTÃO COM CONTROLE DE PERMISSÃO ─────────────────────────────────────────
function PermBtn({canEdit, children, onClick, variant="primary", small, full, style:st={}}){
  if(canEdit){
    return <Btn onClick={onClick} variant={variant} small={small} full={full} style={st}>{children}</Btn>;
  }
  return (
    <NoPermTooltip>
      <Btn onClick={undefined} variant={variant} small={small} full={full} disabled style={{...st, opacity:0.3, cursor:"not-allowed", filter:"grayscale(0.6)"}}>{children}</Btn>
    </NoPermTooltip>
  );
}
...
function DualProgress({dur,mesExec,mesPago,valor,tipo,valorPago}){
  dur=Math.max(+dur||1,1); mesExec=Math.min(Math.max(+mesExec||0,0),dur); mesPago=Math.min(Math.max(+mesPago||0,0),dur);
  const pctExec=(mesExec/dur)*100; const pctPago=(mesPago/dur)*100;
  ...
  return <div style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:14,padding:16,display:"flex",flexDirection:"column",gap:14}}>
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:cPago}}/><span style={{fontSize:12,fontWeight:600,color:T.text}}>💰 Pagamento recebido</span></div>
        <span style={{fontSize:14,fontWeight:800,color:cPago,fontFamily:M}}>{mesPago} / {dur} meses · {fmt(valorRecebido)}</span>
      </div>
      <div style={{background:T.border,borderRadius:6,height:10,overflow:"hidden",marginBottom:4}}><div style={{background:cPago,width:`${pctPago}%`,height:"100%",borderRadius:6,transition:"width 0.5s"}}/></div>
      <div style={{fontSize:11,color:T.muted}}>Entrada no caixa: {fmt(valorRecebido)} de {fmt(valorTotal)}</div>
    </div>
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{width:8,height:8,borderRadius:"50%",background:cExec}}/><span style={{fontSize:12,fontWeight:600,color:T.text}}>⚙️ Execução / entrega</span></div>
        <span style={{fontSize:14,fontWeight:800,color:cExec,fontFamily:M}}>{mesExec} / {dur} meses</span>
      </div>
      <div style={{background:T.border,borderRadius:6,height:10,overflow:"hidden",marginBottom:4}}><div style={{background:cExec,width:`${pctExec}%`,height:"100%",borderRadius:6,transition:"width 0.5s"}}/></div>
      <div style={{fontSize:11,color:T.muted}}>Entregue: {fmt(valorExec)} de {fmt(valorTotal)}</div>
    </div>
    <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
      {Array.from({length:dur},(_,i)=>{const pago=i<mesPago;const exec=i<mesExec;return <div key={i} title={`Mês ${i+1}${pago?" · Pago":""}${exec?" · Executado":""}`} style={{width:28,height:28,borderRadius:"50%",background:pago&&exec?T.green:pago?T.purple:exec?cExec:T.border,display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,color:(pago||exec)?"#fff":T.muted,fontWeight:700,border:pago&&exec?`2px solid ${T.green}33`:"none"}}>{i+1}</div>;})}
    </div>
...
function LancamentosTab({clienteId,clienteNome,caixaAdd,canEdit}){
  ...
  const save=async()=>{
    ...
    if(caixaAdd) await caixaAdd({data:form.data,descricao:`${clienteNome} — ${form.descricao}`,tipo:form.tipo==="Receita"?"Entrada":"Saída",categoria:form.categoria,valor:+form.valor});
    setModal(false);
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <PermBtn canEdit={canEdit} small onClick={()=>setModal(true)}>+ Lançamento avulso</PermBtn>
    </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      {[...meus].sort((a,b)=>(b.data||"").localeCompare(a.data||"")).map(l=><div key={l.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",background:T.surface,border:`1px solid ${T.border}`,borderRadius:10,padding:"10px 14px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
          ...
          <PermBtn canEdit={canEdit} variant="danger" small onClick={()=>{if(window.confirm(`Remover lançamento "${l.descricao}"?`))remove(l.id);}}>✕</PermBtn>
        </div>
      </div>)}
    </div>}
    {modal&&canEdit&&<Modal title="Lançamento avulso" onClose={()=>setModal(false)}>
...
function FichaCliente({item,isNew,onSave,onClose,caixaAdd,canEdit}){
  const [form,setForm]=useState({...EMPTY_C,...item,status_saude:["verde","amarelo","vermelho"].includes(item?.status_saude)?item.status_saude:"verde",duracao_meses:Math.max(+item?.duracao_meses||1,1),mes_atual:Math.max(+item?.mes_atual||0,0),meses_pagos:Math.max(+item?.meses_pagos||0,0),valor_total_pago:+item?.valor_total_pago||0,satisfacao:+item?.satisfacao||3,valor_mensal:item?.valor_mensal||""});
  ...
  return <Modal title="" onClose={onClose} wide>
    {!canEdit&&<div style={{background:"#f59e0b12",border:"1px solid #f59e0b33",borderRadius:10,padding:"10px 16px",marginBottom:16,fontSize:12,color:T.amber,display:"flex",alignItems:"center",gap:8}}>
      🔒 <span>Você está no modo <strong>somente visualização</strong>. Apenas admins podem editar.</span>
    </div>}
    <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24,paddingBottom:20,borderBottom:`1px solid ${T.border}`}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        <input value={form.nome||""} onChange={set("nome")} placeholder="Nome do cliente" disabled={!canEdit}
          style={{background:"transparent",border:"none",outline:"none",fontSize:20,fontWeight:700,color:T.text,fontFamily:F,width:"100%",cursor:canEdit?"text":"default"}}/>
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
      <Inp label="Contato" value={form.contato||""} onChange={set("contato")} disabled={!canEdit}/>
      <Inp label="Email" type="email" value={form.email||""} onChange={set("email")} disabled={!canEdit}/>
      <Inp label="Telefone" value={form.telefone||""} onChange={set("telefone")} disabled={!canEdit}/>
      <Inp label="Segmento" value={form.segmento||""} onChange={set("segmento")} disabled={!canEdit}/>
      <Sel label="Status" value={form.status||"Ativo"} onChange={set("status")} options={["Ativo","Inativo","Pausado"]} disabled={!canEdit}/>
      <Inp label="Início do contrato" type="date" value={form.inicio||""} onChange={set("inicio")} disabled={!canEdit}/>
    </div>
    ...
    {tab==="contrato"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <Inp label="Valor mensal (R$)" type="number" value={form.valor_mensal||""} onChange={set("valor_mensal")} disabled={!canEdit}/>
        <Sel label="Tipo de contrato" value={form.tipo_contrato||"Fee Mensal"} onChange={set("tipo_contrato")} options={[{value:"Fee Mensal",label:"Fee Mensal (recorrente)"},{value:"Unitário",label:"Unitário (projeto com prazo)"}]} disabled={!canEdit}/>
        <Sel label="Duração total" value={form.duracao_meses||1} onChange={set("duracao_meses")} options={[1,2,3,6,12,24].map(n=>({value:n,label:`${n} ${n===1?"mês":"meses"}`}))} disabled={!canEdit}/>
        <Sel label="Mês de execução atual" value={form.mes_atual||0} onChange={set("mes_atual")} options={Array.from({length:(+form.duracao_meses||1)+1},(_,i)=>({value:i,label:i===0?"Não iniciado":`Mês ${i} de ${form.duracao_meses}`}))} disabled={!canEdit}/>
        <Sel label="Meses pagos pelo cliente" value={form.meses_pagos||0} onChange={async e=>{const novosPagos=+e.target.value;setForm(f=>({...f,meses_pagos:novosPagos,valor_total_pago:(+f.valor_mensal||0)*novosPagos}));}} options={Array.from({length:(+form.duracao_meses||1)+1},(_,i)=>({value:i,label:i===0?"Nenhum pagamento":`${i} ${i===1?"mês pago":"meses pagos"} — ${new Intl.NumberFormat("pt-BR",{style:"currency",currency:"BRL"}).format((+form.valor_mensal||0)*i)}`}))} disabled={!canEdit}/>
        <Inp label="Valor total recebido (R$)" type="number" value={form.valor_total_pago||0} onChange={set("valor_total_pago")} disabled={!canEdit}/>
      </div>
      ...
      <Tex label="Escopo total contratado" full rows={3} value={form.escopo_total||""} onChange={set("escopo_total")} placeholder="O que foi contratado no total..." disabled={!canEdit}/>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div><Inp label="Link da proposta" value={form.proposta_url||""} onChange={set("proposta_url")} placeholder="https://drive.google.com/..." disabled={!canEdit}/>{form.proposta_url&&<a href={form.proposta_url} target="_blank" rel="noreferrer" style={{fontSize:12,color:T.blue,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:4,marginTop:6}}>🔗 Abrir proposta →</a>}</div>
        <div><Inp label="Link do vídeo" value={form.video_url||""} onChange={set("video_url")} placeholder="https://youtube.com/..." disabled={!canEdit}/>{form.video_url&&<a href={form.video_url} target="_blank" rel="noreferrer" style={{fontSize:12,color:T.purple,textDecoration:"none",display:"inline-flex",alignItems:"center",gap:4,marginTop:6}}>▶ Abrir vídeo →</a>}</div>
      </div>
    </div>}
    {tab==="entregas"&&<div style={{display:"flex",flexDirection:"column",gap:14}}>
      <Tex label="O que já entregamos" full rows={5} value={form.entregas||""} onChange={set("entregas")} placeholder="Liste as entregas realizadas até agora..." disabled={!canEdit}/>
      <Tex label="Observações gerais" full rows={3} value={form.observacoes||""} onChange={set("observacoes")} placeholder="Notas importantes sobre este cliente..." disabled={!canEdit}/>
    </div>}
    {tab==="saude"&&<div style={{display:"flex",flexDirection:"column",gap:16}}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
        <div>
          ...
          <Stars value={+form.satisfacao} onChange={canEdit?v=>setForm(f=>({...f,satisfacao:v})):undefined}/>
        ...
        <Sel label="Status de saúde" value={form.status_saude||"verde"} onChange={set("status_saude")} options={[{value:"verde",label:"🟢 Saudável"},{value:"amarelo",label:"🟡 Atenção"},{value:"vermelho",label:"🔴 Crítico"}]} disabled={!canEdit}/>
      </div>
      <Tex label="Como está nosso trabalho para este cliente?" full rows={5} value={form.nota_saude||""} onChange={set("nota_saude")} placeholder="Está entregando resultado? Pontos de atenção? O que precisa melhorar?" disabled={!canEdit}/>
    ...
    {tab==="lancamentos"&&(isNew?<div style={{color:T.muted,fontSize:13,textAlign:"center",padding:"32px 0"}}>Salve o cliente primeiro para adicionar lançamentos.</div>:<LancamentosTab clienteId={item.id} clienteNome={form.nome} caixaAdd={caixaAdd} canEdit={canEdit}/>)}
    <div style={{display:"flex",gap:8,marginTop:24,justifyContent:"flex-end",paddingTop:20,borderTop:`1px solid ${T.border}`}}>
      <Btn variant="ghost" onClick={onClose}>{canEdit?"Cancelar":"Fechar"}</Btn>
      {canEdit&&<Btn onClick={async()=>{
        ...
        if(caixaAdd&&novoMesPago>prevMesPago){
          ...
          await caixaAdd({data:today(),descricao:`${form.nome} — ${mesesNovos} ${mesesNovos===1?"mensalidade":"mensalidades"} recebida(s)`,tipo:"Entrada",categoria:"Receita de Serviço",valor:valorNovo});
        }
        setSaving(false);
...
// ─── CLIENTES PAGE ────────────────────────────────────────────────────────────
function Clientes({caixaAdd,canEdit}){
  ...
  const salvar=async form=>{
    const data={...form,valor_mensal:+form.valor_mensal||0,satisfacao:+form.satisfacao||3,duracao_meses:+form.duracao_meses||1,mes_atual:+form.mes_atual||0,status_saude:form.status_saude&&["verde","amarelo","vermelho"].includes(form.status_saude)?form.status_saude:"verde"};
    if(isNew) await add(data); else await update(ficha.id,data);
    setFicha(null); reload();
  };
      ...
      {filtered.map(item=>{
        ...
        return <div key={item.id} onClick={()=>abrir(item)}
          ...
          <div style={{display:"flex",justifyContent:"flex-end",marginTop:10}} onClick={e=>e.stopPropagation()}>
            <PermBtn canEdit={canEdit} variant="danger" small onClick={()=>{if(window.confirm(`Remover ${item.nome}? Esta ação não pode ser desfeita.`))remove(item.id);}}>Remover</PermBtn>
          </div>
        </div>;
      })}
    </div>}
    {ficha&&<FichaCliente item={ficha} isNew={isNew} onSave={salvar} onClose={()=>setFicha(null)} caixaAdd={caixaAdd} canEdit={canEdit}/>}
  </div>;
}

// ─── PATRIMÔNIO TAB ───────────────────────────────────────────────────────────
function PatrimonioTab({db,canEdit}){
  const [modal,setModal]=useState(false);
  ...
  const catColors={"Caixa em banco":T.green,"Conta corrente":T.blue,"Investimento CDB/CDI":T.accent,"Poupança":T.amber,"Outros":T.muted};
  return <div style={{display:"flex",flexDirection:"column",gap:20}}>
    ...
    <div style={{display:"flex",justifyContent:"flex-end"}}><PermBtn canEdit={canEdit} small onClick={()=>setModal(true)}>+ Registrar patrimônio</PermBtn></div>
    <Card style={{padding:0}}>
      <Table cols={[
        ...
        {key:"id",label:"",align:"right",render:(v,r)=><div style={{display:"flex",gap:6}}>
          <PermBtn canEdit={canEdit} variant="soft" small onClick={()=>setEdit(r)}>✎</PermBtn>
          <PermBtn canEdit={canEdit} variant="danger" small onClick={()=>{if(window.confirm("Remover este registro? Esta ação não pode ser desfeita."))db.remove(v);}}>✕</PermBtn>
        </div>},
      ]} rows={db.rows}/>
    </Card>
    {modal&&canEdit&&<Modal title="Registrar patrimônio" onClose={()=>setModal(false)}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
    ...
    {edit&&canEdit&&<Modal title="Editar patrimônio" onClose={()=>setEdit(null)}>
...
// ─── FINANCEIRO ───────────────────────────────────────────────────────────────
function Financeiro({caixaDB,custosDB,invDB,canEdit}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    {aba==="caixa"&&<CaixaTab db={caixaDB} canEdit={canEdit}/>}
    {aba==="custos"&&<CustosTab db={custosDB} canEdit={canEdit}/>}
    {aba==="investimentos"&&<InvestTab db={invDB} canEdit={canEdit}/>}
    {aba==="patrimonio"&&<PatrimonioTab db={patrimonioDB} canEdit={canEdit}/>}
    {aba==="dre"&&<DRETab caixa={caixaDB.rows} custos={custosDB.rows} inv={invDB.rows}/>}
  </div>;
}

function CaixaTab({db,canEdit}){
  const [modal,setModal]=useState(false);
  const [edit,setEdit]=useState(null);
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"flex",justifyContent:"flex-end"}}><PermBtn canEdit={canEdit} small onClick={()=>setModal(true)}>+ Lançamento manual</PermBtn></div>
    <Card style={{padding:0}}>
      <Table cols={[
        {key:"data",label:"Data"},
    ...
    {modal&&canEdit&&<Modal title="Lançamento manual" onClose={()=>setModal(false)}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
        <Inp label="Data" type="date" value={form.data} onChange={s("data")}/>
    ...
    {edit&&canEdit&&<Modal title="Editar lançamento" onClose={()=>setEdit(null)}>
...
function CustosTab({db,canEdit}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <PermBtn canEdit={canEdit} small onClick={()=>setModal(true)}>+ Custo</PermBtn>
    </div>
    <Card style={{padding:0}}>
      <Table cols={[
    ...
    {modal&&canEdit&&<Modal title="Novo Custo" onClose={()=>setModal(false)}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
    ...
    {edit&&canEdit&&<Modal title="Editar custo" onClose={()=>setEdit(null)}><QuickEdit item={edit} fields={[{k:"nome",l:"Descrição",full:true},{k:"categoria",l:"Categoria",t:"select",opts:["Pessoal","Aluguel","TI / Software","Marketing","Administrativo","Logística","Outros"]},{k:"valor",l:"Valor (R$)",t:"number"},{k:"mes",l:"Mês",t:"month"}]} onSave={saveEdit} onClose={()=>setEdit(null)}/></Modal>}
  </div>;
}

function InvestTab({db,canEdit}){
  const [modal,setModal]=useState(false);
  const [edit,setEdit]=useState(null);
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:14}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <PermBtn canEdit={canEdit} small onClick={()=>setModal(true)}>+ Novo investimento</PermBtn>
    </div>
    <Card style={{padding:0}}>
      <Table cols={[
    ...
    {modal&&canEdit&&<Modal title="Novo investimento na empresa" onClose={()=>setModal(false)}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
    ...
    {edit&&canEdit&&<Modal title="Editar" onClose={()=>setEdit(null)}><QuickEdit item={edit} fields={[{k:"descricao",l:"Descrição",full:true},{k:"categoria",l:"Categoria",t:"select",opts:["Patrimônio em Banco","Equipamento","Capacitação","Infraestrutura","Marketing","Tecnologia","Outros"]},{k:"valor",l:"Valor (R$)",t:"number"},{k:"data",l:"Data",t:"date"},{k:"retorno_esperado",l:"Retorno / Obs.",full:true}]} onSave={saveEdit} onClose={()=>setEdit(null)}/></Modal>}
  </div>;
}
...
// ─── CRM ──────────────────────────────────────────────────────────────────────
function CRM({clientesAdd,canEdit}){
  ...
  const moveEtapa=async(id,etapa)=>{
    ...
    if(etapa==="Fechado"&&r.etapa!=="Fechado"&&clientesAdd){
      ...
      const criado=await clientesAdd({...EMPTY_C,nome:r.empresa,contato:r.contato||"",email:r.email||"",telefone:r.telefone||"",valor_mensal:+r.valor_estimado||0,tipo_contrato:r.tipo_contrato||"Fee Mensal",duracao_meses:+r.duracao_meses||1,mes_atual:0,inicio:today(),status:"Ativo",status_saude:"verde",satisfacao:3});
      if(criado) alert("✓ "+r.empresa+" foi cadastrado automaticamente em Clientes!");
    }
  };
  const fecharComoCliente=async(lead)=>{
    if(!canEdit||!clientesAdd) return;
    if(!window.confirm(`Fechar ${lead.empresa} e criar como cliente? Isso vai cadastrá-lo automaticamente na aba Clientes.`)) return;
    const criado=await clientesAdd({...EMPTY_C,nome:lead.empresa||"",contato:lead.contato||"",email:lead.email||"",telefone:lead.telefone||"",segmento:"",valor_mensal:+lead.valor_estimado||0,tipo_contrato:lead.tipo_contrato||"Fee Mensal",duracao_meses:Math.max(+lead.duracao_meses||1,1),mes_atual:0,inicio:today(),status:"Ativo",status_saude:"verde",satisfacao:3,escopo_total:"",entregas:"",proposta_url:"",video_url:"",nota_saude:"",observacoes:""});
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
      ...
      <div style={{display:"flex",gap:8}}>
        ...
        <PermBtn canEdit={canEdit} onClick={()=>setModal(true)}>+ Novo lead</PermBtn>
      </div>
    </div>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:12}}>
        ...
        return <div key={etapa} style={{minWidth:230,flex:"0 0 230px",background:T.surface,border:`1px solid ${T.border}`,borderRadius:14,padding:14}}>
          ...
          {etLeads.map(lead=><div key={lead.id} style={{background:T.card,border:`1px solid ${T.border}`,borderRadius:12,padding:12,marginBottom:8}}>
            ...
            <div style={{display:"flex",gap:4,marginTop:10,flexWrap:"wrap"}}>
              <PermBtn canEdit={canEdit} variant="soft" small onClick={()=>setEdit(lead)}>✎</PermBtn>
              {canEdit
                ...
                :<select disabled value={lead.etapa} style={{background:T.surface,border:`1px solid ${T.border}`,borderRadius:6,color:T.muted,fontSize:11,padding:"4px 6px",fontFamily:F,cursor:"not-allowed",flex:1,opacity:0.5}}>
                  {ETAPAS.map(e=><option key={e} value={e}>{e}</option>)}
                </select>
              }
              <PermBtn canEdit={canEdit} variant="danger" small onClick={()=>{if(window.confirm(`Remover lead ${lead.empresa}?`))remove(lead.id);}}>✕</PermBtn>
            </div>
            {lead.etapa==="Negociação"&&<PermBtn canEdit={canEdit} variant="success" small full style={{marginTop:8}} onClick={()=>fecharComoCliente(lead)}>✓ Fechar e virar cliente</PermBtn>}
          </div>)}
        </div>;
      })}
    </div>:
    <Card style={{padding:0}}>
      <Table cols={[
        ...
        {key:"id",label:"",align:"right",render:(v,r)=><div style={{display:"flex",gap:6}}>
          {r.etapa==="Negociação"&&<PermBtn canEdit={canEdit} variant="success" small onClick={()=>fecharComoCliente(r)}>✓ Fechar</PermBtn>}
          <PermBtn canEdit={canEdit} variant="soft" small onClick={()=>setEdit(r)}>✎</PermBtn>
          <PermBtn canEdit={canEdit} variant="danger" small onClick={()=>{if(window.confirm(`Remover lead ${r.empresa}?`))remove(v);}}>✕</PermBtn>
        </div>},
      ]} rows={filtered}/>
    </Card>}
    {modal&&canEdit&&<Modal title="Novo Lead" onClose={()=>setModal(false)} wide>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
    ...
    {edit&&canEdit&&<Modal title={`Editar: ${edit.empresa}`} onClose={()=>setEdit(null)} wide>
...
function Dashboard({clientes,caixa,custos,investimentos,leads,lancamentos,patrimonio}){
  ...
  return <div style={{display:"flex",flexDirection:"column",gap:24}}>
    ...
    {(criticos.length>0||atencao.length>0)&&<div style={{background:T.redBg,border:`1px solid ${T.red}33`,borderRadius:14,padding:"14px 18px",display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>{[...criticos,...atencao].map(c=><Badge key={c.id} color={SAUDE[c.status_saude].color}>{SAUDE[c.status_saude].emoji} {c.nome}</Badge>)}</div>
    </div>}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 2fr",gap:16}}>
      <Card>
        ...
        {[["verde","🟢 Saudáveis",T.green],["amarelo","🟡 Atenção",T.amber],["vermelho","🔴 Críticos",T.red]].map(([k,l,c])=><div key={k} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:60}}><MiniBar value={porSaude(k)} max={ativos.length||1} color={c}/></div><span style={{fontSize:18,fontWeight:800,color:c,fontFamily:M,minWidth:24,textAlign:"right"}}>{porSaude(k)}</span></div>
        </div>)}
    ...
    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
      <Card>
        ...
        [...criticos,...atencao].slice(0,5).map(c=>{const s=SAUDE[c.status_saude]||SAUDE.verde;return <div key={c.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,padding:"10px 14px",background:s.bg,borderRadius:10,border:`1px solid ${s.color}22`}}>
          ...
          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}><Badge color={s.color}>{s.emoji}</Badge><span style={{fontSize:11,color:T.muted,fontFamily:M}}>{fmt(c.valor_mensal)}/mês</span></div>
        </div>;})}
      </Card>
      <Card>
        ...
        [...ativos].sort((a,b)=>(+b.valor_mensal||0)-(+a.valor_mensal||0)).slice(0,5).map((c,i)=><div key={c.id} style={{display:"flex",alignItems:"center",gap:12,marginBottom:12}}>
          ...
          <div style={{flex:1,minWidth:0}}><div style={{fontSize:13,color:T.text,marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{c.nome}</div><MiniBar value={+c.valor_mensal||0} max={Math.max(...ativos.map(x=>+x.valor_mensal||0),1)} color={T.accent}/></div>
...
export default function App(){
  ...
  // ─── PERMISSÃO: calculada a partir do email do usuário logado ─────────────
  const canEdit = canEditUser(user?.email);

  const caixaAdd=async(row)=>{ return await caixaDB.add(row); };
  const clientesAdd=async(row)=>{ return await clientesDB.add(row); };
  ...
  return <div style={{display:"flex",minHeight:"100vh",background:T.bg,fontFamily:F,color:T.text}}>
    ...
    <aside style={{width:collapsed?60:220,background:T.surface,borderRight:`1px solid ${T.border}`,display:"flex",flexDirection:"column",transition:"width 0.2s",flexShrink:0,overflow:"hidden"}}>
      ...
      {!collapsed&&<div style={{padding:"14px 20px",borderTop:`1px solid ${T.border}`}}>
        <div style={{fontSize:11,color:T.sub,marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>👤 {user?.email||""}</div>
        <div style={{fontSize:10,color:canEdit?T.green:T.amber,marginBottom:8,display:"flex",alignItems:"center",gap:4}}>
          {canEdit?"🔓 Admin · pode editar":"🔒 Somente visualização"}
        </div>
